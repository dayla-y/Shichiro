...
export class StateMachine {
    ...
    public setState(name: string, ...args: unknown[]): void{
        ...
        if(!this.#states.has(name)){
            console.log(`[${StateMachine.name}-${this.#id}:${methodName}] tried to change to unknow state ${name}`);
            return;
        }
        ...
        this.#log(methodName, `change from ${this.#currentState?.name ?? 'name'} to ${name}`)
